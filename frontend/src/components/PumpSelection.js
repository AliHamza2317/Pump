...
const Balance = ({ goBack }) => {
  ...
  const handleAddSale = async () => {
    if (depositDate && saleDate && name && site && amount) {
      ...
      try {
        const response = await fetch('http://localhost:3002/credits', {
          method: 'POST',
          headers: {
  ...
  const handleAddDebit = async () => {
    if (debitDate && companyName && debitSite && debitAmount) {
      ...
      try {
        const response = await fetch('http://localhost:3002/debits', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
...
const AmountSection = ({ pumpName, goBack }) => {
  ...
  const saveAmount = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3002/amounts', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
const clearInputFields = (type) => {
  ...
  const handleStockUpdate = async (type, date, delivery, sale) => {
    ...
    try {
        const response = await fetch('http://localhost:3002/pumps', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
const UdharSection = ({ pumpName, goBack }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3002/udhars', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
